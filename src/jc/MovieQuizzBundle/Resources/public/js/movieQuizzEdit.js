...
/**
 * JS script used in BO to edit a quizz => see view jcMovieQuizzBundle:BO:editQuizz.html.twig
 */
...
function loadResponseForQuizz() {
    ...
    var ajaxBaseUrl = global.basePath + '/admin/movie-quizz/' + quizzId + '/response/list';
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#movie-quizz-responses-list').html(response);
...
function defineActionForResponses() {
    ...
    $('#movie-quizz-response-list img.movie-quizz-edit-response').each(function() {
...
function editResponse(quizzId, responseId) {
    ...
    var ajaxBaseUrl = global.basePath + '/admin/movie-quizz/' + quizzId + '/response/edit/' + responseId;
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#movie-quizz-response-popup').html(response);
            initSubmitResponseForm();
            $('#movie-quizz-response-popup').bPopup();
        },
        error: function(msg) {
...
function deleteResponse(button) {
    ...
    if (confirm('Confirmer la suppression ?')) {
        ...
        var ajaxBaseUrl = global.basePath + '/admin/movie-quizz/' + quizzId + '/response/delete';
...
function initSubmitResponseForm() {
    ...
    $("#movie-quizz-response-form").submit(function(event){

        global.cancelEvent(event);

        var ajaxBaseUrl = $("#movie-quizz-response-form").attr('action');
        var formSerialize = $("#movie-quizz-response-form").serialize();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#movie-quizz-response-popup').bPopup().close();
                    loadResponseForQuizz();
                }
                else
                    alert(response.message);
